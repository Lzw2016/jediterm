...
//        setFocusable(false);
    ...
    public static Image getComponentImage(TabInfo info) {
        ...
        if (cmp.isShowing()) {
            ...
            img = DrawUtil.createImage(width > 0 ? width : 500, height > 0 ? height : 500, BufferedImage.TYPE_INT_ARGB);
            Graphics2D g = img.createGraphics();
            cmp.paint(g);
        } else {
            img = DrawUtil.createImage(500, 500, BufferedImage.TYPE_INT_ARGB);
        }
        return img;
    }
...
//todo [kirillk] dirty hack, should rely on ActionManager to understand that menu item was either chosen on or cancelled
        ...
        if (blocked && !myPaintBlocked) {
            if (takeSnapshot) {
                if (getWidth() > 0 && getHeight() > 0) {
                    myImage = DrawUtil.createImage(getWidth(), getHeight(), BufferedImage.TYPE_INT_ARGB);
    ...
    protected void paintSelectionAndBorder(Graphics2D g2d) {
        ...
        if (paintFocused) {
            ...
            if (isPaintFocus()) {
                if (bgColor == null) {
                    alpha = 150;
                    shapeInfo.from = ColorUtil.toAlpha(DrawUtil.getPanelBackground().brighter(), alpha);
                    shapeInfo.to = ColorUtil.toAlpha(DrawUtil.getPanelBackground(), alpha);
                } else {
                    alpha = 255;
        ...
        if (!isHideTabs()) {
            ...
            g2d.setPaint(DrawUtil.getGradientPaint((float) gradientLine.getX1(), (float) gradientLine.getY1(),
    ...
    private void paintNonSelected(final Graphics2D g2d,
        ...
        if (isToBufferPainting()) {
            ...
            if (img == null) {
                img = DrawUtil.createImage(width, height, BufferedImage.TYPE_INT_ARGB);
    ...
    public Color getBackground() {
        return UIUtil.isUnderNimbusLookAndFeel() ? DrawUtil.getPanelBackground() : UIUtil.getBgFillColor(getParent());
    }
    ...
    protected void doPaintInactive(Graphics2D g2d,
        ...
        final Paint gp = DrawUtil.isUnderDarcula()
                ? DrawUtil.getGradientPaint(gradientLine.x1, gradientLine.y1,
