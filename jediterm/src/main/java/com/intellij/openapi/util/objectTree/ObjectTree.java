...
public final class ObjectTree<T> {
    ...
    private final Map<Object, Object> myDisposedObjects = new WeakHashMap<Object, Object>(100, 0.5f); // guarded by treeLock
    ...
    public final void register(@NotNull T parent, @NotNull T child) {
        ...
        if (wasDisposed != null) {
            throw new IllegalStateException("Sorry but parent: " + parent + " has already been disposed " +
    ...
    private void checkWasNotAddedAlready(ObjectNode<T> childNode, @NotNull ObjectNode<T> parentNode) {
        for (ObjectNode<T> node = childNode; node != null; node = node.getParent()) {
            if (node == parentNode) {
                throw new IllegalStateException("'" + childNode.getObject() + "' was already added as a child of '" + parentNode.getObject() + "'");
            }
        }
    }
