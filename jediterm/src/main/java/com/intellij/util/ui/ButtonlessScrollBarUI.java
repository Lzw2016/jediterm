...
public class ButtonlessScrollBarUI extends BasicScrollBarUI {
    ...
    private JBColor getTrackBorderColor() {
        return jbColor(com.intellij.ui.Gray._230, UIUtil.getListBackground());
    }
    ...
    private void startMacScrollbarFadeout(boolean now) {
        ...
        if (sb != null) {
            ...
            if (!myMouseOverScrollbar && !sb.getValueIsAdjusting()) {
                myMacScrollbarFadeTimer.addRequest(() -> myMacScrollbarFadeAnimator.resume(), 700);
            }
        }
    }
    ...
    private void paintMacThumb(Graphics g, Rectangle thumbBounds) {
        ...
        JBColor baseColor = new JBColor(() -> !isDark() ? com.intellij.ui.Gray._0 : com.intellij.ui.Gray._128);
    ...
    protected Color adjustColor(Color c) {
        if (isMacOverlayScrollbar()) {
            ...
            return com.intellij.ui.Gray.get(Math.max(0, Math.min(255, c.getRed() - sign * myThumbFadeColorShift)));
        }
    }
    ...
    public static class ThumbPainter implements RegionPainter<Integer> {
        ...
        private static Color getColor(int gray) {
            return com.intellij.ui.Gray.get(gray < 0 ? 0 : gray > 255 ? 255 : gray);
        }
    }
